...
let ApprovalsService = class ApprovalsService {
    ...
    async listPendingForUser(userId, roles) {
        ...
        const documents = await this.documents.find({ where: { id: (0, typeorm_2.In)(requests.map((request) => request.entityId)) } });
    ...
    async notifyApprovalResult(request, document, result) {
        ...
        await this.notifications.notify({
            recipients: [{ userId: request.requesterId }, document.responsibleUserId ? { userId: document.responsibleUserId } : null].filter((item) => Boolean(item?.userId)),
