...
let NotificationsService = (NotificationsService_1 = class NotificationsService {
  ...
  async updatePreferences(userId, items) {
    const existing = await this.preferences.find({
      where: {
        userId,
        notificationType: (0, typeorm_2.In)(items.map((item) => item.notificationType)),
      },
    });
  ...
  async notify(payload) {
    ...
    const users = await this.users.find({
      where: { id: (0, typeorm_2.In)([...uniqueRecipients.keys()]), active: true },
    });
    const preferences = await this.preferences.find({
      where: { userId: (0, typeorm_2.In)(users.map((user) => user.id)) },
    });
  ...
  NotificationsService_1 =
    __decorate(
      [
        ...
        __param(
          1,
          (0, typeorm_1.InjectRepository)(notification_preference_entity_1.NotificationPreference)
        ),
        __param(
          2,
          (0, typeorm_1.InjectRepository)(notification_delivery_entity_1.NotificationDelivery)
        ),
        ...
        __param(
          8,
          (0, typeorm_1.InjectRepository)(contract_obligation_entity_1.ContractObligation)
        ),
