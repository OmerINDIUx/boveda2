...
let AiQueryService = class AiQueryService {
    ...
    indexing;
    audit;
    constructor(documents, versions, chunks, permissions, history, users, members, scope, indexing, audit) {
    ...
    async buildGroundedResponse(visibleDocuments, question) {
        ...
        const searchResults = await this.indexing.searchVisibleChunks(visibleDocumentsWithVersion.map((document) => document.id), question);
        const citedChunks = await this.toCitations(searchResults, visibleDocumentsWithVersion, versionsById);
        if (!citedChunks.length) {
            return {
                status: 'insufficient_information',
                answer: 'No encontre informacion suficiente en los documentos autorizados para responder con seguridad. Intenta reformular la pregunta o acotarla a un documento especifico.',
                citations: []
            };
        }
        return {
    ...
    composeAnswer(question, citations) {
        ...
        return citations
            .slice(0, 4)
            .map((citation, index) => `${index + 1}. ${citation.documentName} (version ${citation.versionLabel}): ${citation.fragment}`)
            .join('\n');
    }
