    ...
    return rows
      .map((embedding) => ({
        ...
        score:
          ...
          this.keywordBoost(question, embedding.chunk.content),
      }))
...
`;
  ...
  async runPython(executable, code, args, stdin) {
    return new Promise((resolve, reject) => {
      const child = (0, child_process_1.spawn)(executable, ['-c', code, ...args], {
      ...
      child.on('error', (error) =>
        reject(
          new common_1.InternalServerErrorException(
            `No fue posible iniciar Python: ${error.message}`
          )
        )
      );
