...
const project_catalog_option_entity_1 = require("./project-catalog-option.entity");
...
let ProjectsService = ProjectsService_1 = class ProjectsService {
    ...
    constructor(projects, members, users, disciplines, folders, documents, catalogOptions, scope, audit, config) {
    ...
    async getFormOptions(userId) {
        ...
        if (disciplinesResult.status !== 'fulfilled') {
            this.logger.warn(`No fue posible cargar disciplinas para form-options del usuario ${userId}: ${disciplinesResult.reason instanceof Error
                ? disciplinesResult.reason.message
                : String(disciplinesResult.reason)}`);
        }
        if (catalogOptionsResult.status !== 'fulfilled') {
            this.logger.warn(`No fue posible cargar catalogos para form-options del usuario ${userId}: ${catalogOptionsResult.reason instanceof Error
    ...
    async createCatalogOption(userId, dto) {
        const created = await this.catalogOptions.save(this.catalogOptions.create({
            ...dto,
    ...
    async checkSynonyms(dto) {
        ...
        const model = this.config.get('OLLAMA_MODEL') ?? 'llama3.1';
        ...
        try {
            const response = await fetch(`${baseUrl.replace(/\/$/, '')}/api/chat`, {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: 'Eres un asistente que identifica sinonimos o terminos muy similares en espanol ' +
            ...
            const matched = existingLabels.find((l) => l.toLowerCase().trim() === answer.toLowerCase().trim());
            if (matched) {
            ...
            const response = await fetch(`${baseUrl.replace(/\/$/, '')}/api/chat`, {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: 'Eres un asistente que encuentra terminos semanticamente relacionados ' +
    ...
    async updateCatalogOption(userId, id, dto) {
        const option = await this.catalogOptions.findOne({ where: { id } });
        if (!option) {
            throw new common_1.NotFoundException('Opción de catálogo no encontrada');
        }
    ...
    async deactivateCatalogOption(userId, id) {
        const option = await this.catalogOptions.findOne({ where: { id } });
        if (!option) {
            throw new common_1.NotFoundException('Opción de catálogo no encontrada');
        }
    ...
    async listForUser(userId) {
        ...
        const [projects, members, documents, disciplineCatalog] = await Promise.all([
            this.projects.find({
                where: { id: (0, typeorm_2.In)(projectIds), isDraft: false },
        ...
        return projects.map((project) => this.toProjectSummary(project, membersByProject.get(project.id) ?? [], documentsByProject.get(project.id) ?? [], disciplineMap));
    }
    async getDetail(userId, projectId) {
        ...
        const projectDisciplineCatalog = disciplineCatalog.filter((d) => projectDisciplineIds.has(d.id));
        const filteredFolderList = folderList.filter((f) => !f.disciplineId || projectDisciplineIds.has(f.disciplineId));
        const disciplineMap = new Map(disciplineCatalog.map((discipline) => [discipline.id, discipline]));
    ...
    async create(dto, ownerId) {
        ...
        const project = await this.projects.save(this.projects.create({
            ...dto,
            ownerId,
    ...
    async publishDraft(requesterId, projectId) {
        const project = await this.projects.findOne({ where: { id: projectId } });
        if (!project) {
            throw new common_1.NotFoundException('Proyecto no encontrado');
        }
        if (!project.isDraft) {
            throw new common_1.BadRequestException('El proyecto no es un borrador');
        }
    ...
    async ensureProjectFolderStructure(projectId, userId, disciplineIds) {
        ...
        for (const definition of ROOT_FOLDER_DEFINITIONS) {
            if (!rootsByName.has(definition.name)) {
                const root = await this.folders.save(this.folders.create({
                    projectId,
        ...
        const disciplines = await this.disciplines.find({ where: { id: (0, typeorm_2.In)(disciplineIds) } });
        const disciplineFolders = new Map(existingFolders
        ...
        for (const discipline of disciplines) {
            if (!disciplineFolders.has(discipline.id)) {
                await this.folders.save(this.folders.create({
                    projectId,
    ...
    async queryProjectDocuments(projectId, filters) {
        ...
        if (filters.search) {
            query.andWhere('(document.name LIKE :search OR document.documentNumber LIKE :search OR discipline.name LIKE :search OR folder.name LIKE :search)', { search: `%${filters.search}%` });
        }
...
exports.ProjectsService = ProjectsService = ProjectsService_1 = __decorate([
    ...
    __param(6, (0, typeorm_1.InjectRepository)(project_catalog_option_entity_1.ProjectCatalogOption)),
